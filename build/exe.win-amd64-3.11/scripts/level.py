...
class Level:
	def __init__(self,current_level,surface,create_overworld,change_coins,change_health):
		...
		self.limite_sprites = self.create_tile_group(limite_layout,'limite')
		...
		for row_index, row in enumerate(layout):
			for col_index,val in enumerate(row):
				if val != '-1':
					...
					if type == 'moedas':
						if val == '0': sprite = Moedas(tile_size,x,y,'graphics/moedas/gold',5)
						if val == '1': sprite = Moedas(tile_size,x,y,'graphics/moedas/silver',1)
	...
	def player_setup(self,layout,change_health):
		for row_index, row in enumerate(layout):
			for col_index,val in enumerate(row):
				...
				if val == '1':
					hat_surface = pygame.image.load('graphics/character/coroa.png').convert_alpha()
					sprite = StaticTile(tile_size,x,y,hat_surface)
					self.goal.add(sprite)

	def enemy_collision_reverse(self):
		for enemy in self.enemy_sprites.sprites():
			if pygame.sprite.spritecollide(enemy,self.limite_sprites,False):
				enemy.reverse()
	...
	def check_coin_collisions(self):
		collided_coins = pygame.sprite.spritecollide(self.player.sprite,self.moedas_sprites,True)
		if collided_coins:
	...
	def run(self):
		...
		self.limite_sprites.update(self.world_shift)
